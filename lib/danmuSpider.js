...
class DanmuSpider {
    ...
    for (const componentObj of obj['pageComponentList']) {
      if (componentObj['commonData'] !== undefined) {
        ...
        if (commonData['notice'] !== undefined) {
          searchVodDetail.vod_actor = commonData['notice']
            .replaceAll('演员：', '')
            .replaceAll(' ', '');
        }
        if (commonData['director'] !== undefined) {
          searchVodDetail.vod_director = commonData['director']
            .replaceAll('导演：', '')
            .replaceAll(' ', '');
        }
  ...
  async downloadDanmu(url) {
    ...
    if (response.code === 200) {
      ...
      await req('http://127.0.0.1:9978/cache', {
        method: 'post',
        data: params,
        postType: 'form-data',
      });
  ...
  async search(vodDetail, episodeId) {
    ...
    let searchObj = JSON.parse(
      await this.fetch(this.siteUrl + '/api/search', params, this.getHeader())
    );
