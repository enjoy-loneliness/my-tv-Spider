...
async function detailContent(share_url_list, type_name = '电影') {
  ...
  await getVod(
    ...
    quarkName,
    playVod,
    quarkItems.video_items,
    quarkItems.sub_items,
    type_name
  );
    ...
    for (let index = 0; index < play_foramt_list.length; index++) {
      ...
      for (const video_item of video_item_list) {
        if (video_item.shareIndex === i + 1) {
          vodItems.push(
            video_item.getEpisodeUrl(type_name) + findSubs(video_item.getName(), sub_item_list)
          );
        }
      }
...
function findSubs(name, item_list) {
  ...
  for (const item of sub_item_list) {
    sub_str +=
      '+' + Utils.removeExt(item.getName()) + '@@@' + item.getExt() + '@@@' + item.getFileId();
  }
  return sub_str;
}
