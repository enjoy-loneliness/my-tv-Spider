...
class StarSpider extends Spider {
  ...
  async setHomeVod() {
    let json = await this.fetch(
      this.apiUrl + '/v3/web/api/home?chName=首页',
      null,
      this.getApiHeader()
    );
  ...
  async setCategory(tid, pg, filter, extend) {
    this.limit = 16;
    const param = {
      chName: this.getClassChName(tid),
      page: parseInt(pg),
      pageSize: this.limit,
    };
...
const { spider, __jsEvalReturn } = utilsClassFun(StarSpider);
