...
class BooKanSpider extends Spider {
  ...
  async setCategory(tid, pg, filter, extend) {
    let content = await this.fetch(
      `${this.siteUrl}/voice/book/list?instance_id=25304&page=${pg}&category_id=${tid}&num=24`,
      null,
      this.getHeader()
    );
  ...
  async setDetail(id) {
    let content = await this.fetch(
      `${this.siteUrl}/voice/album/units?album_id=${id}&page=1&num=200&order=1`,
      null,
      this.getHeader()
    );
    let data = JSON.parse(content).data;
  ...
  async setSearch(wd, quick) {
    let content = await this.fetch(
      `https://es.bookan.com.cn/api/v3/voice/book?instanceId=25304&keyword=${wd}&pageNum=1&limitNum=20`,
      null,
      this.getHeader()
    );
    let data = JSON.parse(content).data;
...
const { spider, __jsEvalReturn } = utilsClassFun(BooKanSpider);
