...
class Doll extends Spider {
  ...
  async parseVodDetailFromDoc($, key) {
    ...
    vodDetail.vod_remarks = $(vodElement)
      .find('[class="tag my-1 text-center"]')[0]
      .attribs['href'].replaceAll('/', '');
    ...
    let play_url_2 = decodeURIComponent(
      Crypto.enc.Utf8.stringify(Crypto.enc.Base64.parse(code.join('')))
    );
  ...
  async setClasses() {
    ...
    let navElements = $(
      '[class="list-unstyled topnav-menu d-flex d-lg-block align-items-center justify-content-center flex-fill topnav-menu-left m-0"]'
    ).find('li');
    let index = 1;
...
const { spider, __jsEvalReturn } = utilsClassFun(Doll);
