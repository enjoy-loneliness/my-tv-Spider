...
class YSXZSpider extends Spider {
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_pic = $($('[class="entry-content u-text-format u-clearfix"]')).find(
      'img'
    )[0].attribs['src'];
    ...
    vodDetail.vod_content = Utils.getStrByRegex(/剧情简介(.*?)\n/, text)
      ...
      .replaceAll('·', '');
    let actors = _.map(
      $('div.entry-content.u-text-format.u-clearfix > div:nth-child(10) > div > span > span'),
      n => {
    ...
    let directors = _.map(
      $('div.entry-content.u-text-format.u-clearfix > div:nth-child(6) > div > span'),
      n => {
        return $(n).text().split(' ')[0];
      }
    );
...
const { spider } = utilsClassFun(YSXZSpider);
