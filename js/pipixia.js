...
class PiPiXiaSpider extends Spider {
  ...
  async parseVodShortListFromDoc($) {
    let vod_list = [];
    let vodElements = $($('[class="wow fadeInUp animated"]')).find(
      '[class="public-list-box public-pic-b"]'
    );
    for (const vodElement of vodElements) {
      let vodShort = new VodShort();
      vodShort.vod_id = Utils.getStrByRegex(
        /v\/(.*?).html/,
        $(vodElement).find('a')[0].attribs.href
      );
      ...
      vodShort.vod_pic =
        ...
        Utils.base64Encode(
          this.siteUrl +
            '/' +
            $(vodElement).find('[class="lazy gen-movie-img mask-1"]')[0].attribs['data-original']
        );
  ...
  async parseVodShortListFromDocBySearch($) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_pic =
        ...
        Utils.base64Encode(
          this.siteUrl +
            ...
            Utils.getStrByRegex(
              /url\((.*?)\);/,
              $(vodElement).find('[class="cover"]')[0].attribs.style
            )
        );
      ...
      vodShort.vod_id = Utils.getStrByRegex(
        /v\/(.*?).html/,
        $(vodElement).find('[class="button"]')[0].attribs.href
      );
      vod_list.push(vodShort);
    }
    return vod_list;
  ...
  async parseVodShortListFromJson(obj) {
    ...
    for (const vod_json of obj['list']) {
      ...
      vodShort.vod_pic =
        this.baseProxy + Utils.base64Encode(this.siteUrl + '/' + vod_json['vod_pic']);
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_pic =
      this.siteUrl +
      $(detailElement).find('[class="detail-pic lazy mask-1"]')[0].attribs['data-original'];
    vodDetail.vod_remarks = $(
      $($(detailElement).find('[class="slide-info hide"]')[0]).find(
        '[class="slide-info-remarks"]'
      )[0]
    ).text();
    vodDetail.vod_year = $(
      $($(detailElement).find('[class="slide-info hide"]')[0]).find(
    ...
    vodDetail.vod_director = $(
      $($(detailElement).find('[class="slide-info hide"]')[1]).find('a')
    ).text();
    vodDetail.vod_actor = $(
      $($(detailElement).find('[class="slide-info hide"]')[2]).find('a')
    ).text();
    let type_list = [];
  ...
  async getPlayConfig(element) {
    ...
    let playListConfig = {
      ...
      'pptv': {
        ...
        'des': '',
        'ps': '1',
        'parse': 'http://play.shijie.chat/player/?url=',
      },
      'letv': {
        'show': 'LE虾线',
        'des': '',
  ...
  uic(url, uid) {
    ...
    let decrypted = Crypto.AES.decrypt(url, ut, {
      iv: mm,
  ...
  async setVideoProxy(playUrl) {
    ...
    for (let index = 1; index < pUrls.length; index += 2) {
      pUrls[index] = js2Proxy(
        false,
        this.siteType,
        this.siteKey,
        'hls/' + encodeURIComponent(pUrls[index]),
        {}
      );
    }
    pUrls.push('original');
...
const { spider, __jsEvalReturn } = utilsClassFun(PiPiXiaSpider);
