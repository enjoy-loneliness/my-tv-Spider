...
class CopyManhuaSpider extends Spider {
  ...
  async parseVodDetailFromDoc($, id) {
    ...
    bookDetail.book_pic = $('[class="comicParticulars-left-img loadingIcon"]').find(
      'img'
    )[0].attribs['data-src'];
    ...
    let data = JSON.parse(
      await this.fetch(this.siteUrl + `/comicdetail/${id}/chapters`, null, this.getHeader())
    )['results'];
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let link =
      this.siteUrl +
      `/comics?theme=${extend.theme || ''}&region=${extend.region || ''}&status=${
        extend.status || ''
      }&ordering=${extend.ordering || '-datetime_updated'}`;
    if (page > 1) {
  ...
  async setSearch(wd, quick) {
    let page = 1;
    const link = `${this.siteUrl}/api/kb/web/searcha/comics?offset=${
      page > 1 ? ((page - 1) * 12).toString() : ''
    }&platform=2&limit=12&q=${wd}&q_type=`;
...
const { spider, __jsEvalReturn } = utilsClassFun(CopyManhuaSpider);
