...
class NanGuaSpider extends Spider {
  ...
  getHeader() {
    ...
    return {
      ...
      'sign': Crypto.MD5('c431ea542cee9679#uBFszdEM0oL0JRn@' + t)
        .toString()
        .toLowerCase(),
  ...
  async setClasses() {
    let data = JSON.parse(
      await this.fetch(this.siteUrl + '/api.php/provide/home_nav', null, this.getHeader())
    );
  ...
  async setHomeVod() {
    let data = JSON.parse(
      await this.fetch(
        this.siteUrl +
          '/api.php/provide/home_data?app=ylys&devices=android&imei=c431ea542cee9679&deviceModel=Subsystem%20for%20Android(TM)&deviceVersion=33&appVersionName=1.0.9&deviceScreen=427*250&appVersionCode=9&deviceBrand=Windows',
        null,
        this.getHeader()
      )
    );
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let reqUrl =
      this.siteUrl +
      '/api.php/provide/vod_list?app=ylys&id=' +
      tid +
      '&page=' +
      pg +
      '&imei=c431ea542cee9679&';
    reqUrl += jinja2(
      'area={{ext.area}}&year={{ext.year}}&type={{ext.class}}&total={{ext.total}}&order={{ext.by}}',
      { ext: extend }
    );
  ...
  async setDetail(id) {
    let data = JSON.parse(
      await this.fetch(
        this.siteUrl + '/api.php/provide/vod_detail?app=ylys&imei=c431ea542cee9679&id=' + id,
        null,
        this.getHeader()
      )
    ).data;
  ...
  async setSearch(wd, quick, pg) {
    ...
    let data = JSON.parse(
      await this.fetch(
        this.siteUrl +
          '/api.php/provide/search_result_more?app=ylys&video_name=' +
          wd +
          `&pageSize=${limit}&tid=0&imei=c431ea542cee9679&page=${pg}`,
        null,
        this.getHeader()
      )
    ).data;
...
const { spider, __jsEvalReturn } = utilsClassFun(NanGuaSpider);
