...
class BQQSpider extends Spider {
  ...
  async search(wd, quick) {
    ...
    const set_cookie = _.isArray(cook.headers['set-cookie'])
      ? cook.headers['set-cookie'].join(';;;')
      : cook.headers['set-cookie'];
    ...
    const resp = await req(
      `${this.siteUrl}/user/search.html?q=${encodeURIComponent(wd)}&so=undefined`,
      {
        headers: {
          accept: 'application/json',
          'User-Agent': Utils.MOBILEUA,
...
const { spider, __jsEvalReturn } = utilsClassFun(BQQSpider);
