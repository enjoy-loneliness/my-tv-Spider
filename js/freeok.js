...
class OkSpider extends Spider {
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_content = $($('[class="module-info-introduction-content"]'))
      .text()
      .replaceAll('\n', '')
      .replaceAll(' ', '');
    let type_list = [];
    ...
    for (let i = 0; i < playElements.length; i++) {
      ...
      if (text.indexOf('夸克') === -1) {
        ...
        for (const playDetailElement of playDetailElements) {
          let play_name = playDetailElement.attribs['title']
            .replaceAll('播放', '')
            .replaceAll(vodDetail.vod_name, '');
  ...
  async getFilter($) {
    ...
    for (let i = 0; i < titleElements.length; i++) {
      let extend_dic = {
        'key': (i + 1).toString(),
        'name': $(titleElements[i]).text(),
        'value': [],
      };
      let typeElements = $(boxElements[i]).find('a');
      for (let j = 0; j < typeElements.length; j++) {
        let type_name = $(typeElements[j]).text();
        let type_id = decodeURIComponent(
          typeElements[j].attribs['href'].split('-')[type_id_dic[$(titleElements[i]).text()]]
        ).replaceAll('.html', '');
        extend_dic['value'].push({
          'n': type_name,
          'v': $(titleElements[i]).text() + '-' + type_id,
        });
      }
  ...
  async setPlay(flag, id, flags) {
    ...
    let view_port_id = Utils.getStrByRegex(/<meta name="viewport"(.*?)>/, playHtml)
      .split('id="')[1]
      .replaceAll('now_', '');
    let player_id = Utils.getStrByRegex(/meta charset="UTF-8" id="(.*?)">/, playHtml).replaceAll(
      'now_',
      ''
    );
...
const { spider, __jsEvalReturn } = utilsClassFun(OkSpider);
