...
class DyttSpider extends Spider {
  ...
  async getHtml(url = this.siteUrl, headers = this.getHeader()) {
    try {
      ...
      if (
        !_.isEmpty(html) &&
        Utils.getStrByRegex(/<script src="(.*?)"><\/script>/, html) !== '/_guard/auto.js'
      ) {
        return load(html);
      } else {
  ...
  async parseVodShortListFromDocByCategory($) {
    ...
    for (const vodShortElement of vodShortElements) {
      ...
      vodShort.vod_remarks =
        '日期:' +
        Utils.getStrByRegex(/日期：(.*?) /, $(vodElements[2]).text()) +
        ' 热度:' +
        Utils.getStrByRegex(/点击：(.*?) /, $(vodElements[2]).text());
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_name = Utils.getStrByRegex(
      /《(.*?)》/,
      Utils.getStrByRegex(/<title>(.*?)<\/title>/, $.html())
    );
