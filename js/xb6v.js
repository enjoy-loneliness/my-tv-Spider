...
class Xb6vSpider extends Spider {
  ...
  async redirect(response) {
    ...
    if (matcher.length > 1) {
      let new_url =
        this.siteUrl + '/e/search/' + matcher[1].replaceAll('\\', '').replaceAll('"', '');
  ...
  async parseVodShortListFromDoc($) {
    ...
    for (const item of items) {
      ...
      vodShort.vod_remarks = $(item)
        .find('[rel="category tag"]')
        .text()
        .replaceAll('\n', '')
        .replaceAll(' ', '');
  ...
  async parseVodDetailFromDoc($) {
    ...
    if (_.isEmpty(vodDetail.vod_year))
      vodDetail.vod_year = Utils.getStrByRegex(/首播:(.*?)<br>"/, partHTML);
    vodDetail.vod_area = Utils.getStrByRegex(/◎产　　地　(.*?)<br>/, partHTML);
    if (_.isEmpty(vodDetail.vod_year))
      vodDetail.vod_area = Utils.getStrByRegex(/地区:(.*?)<br>"/, partHTML);
    vodDetail.vod_remarks = Utils.getStrByRegex(/◎上映日期　(.*?)<br>/, partHTML);
    vodDetail.vod_actor = this.getActorOrDirector(/◎演　　员　(.*?)<\/p>/, partHTML);
    if (_.isEmpty(vodDetail.vod_actor))
      vodDetail.vod_actor = this.getActorOrDirector(/◎主　　演　(.*?)<\/p>/, partHTML);
    if (_.isEmpty(vodDetail.vod_actor))
      vodDetail.vod_actor = this.getActorOrDirector(/主演:(.*?)<br>/, partHTML);
    vodDetail.vod_director = this.getActorOrDirector(/◎导　　演　(.*?)<br>/, partHTML);
    if (_.isEmpty(vodDetail.vod_director))
      vodDetail.vod_director = this.getActorOrDirector(/导演:(.*?)<br>/, partHTML);
    vodDetail.vod_content = this.getDescription(/◎简　　介(.*?)<hr>/gi, partHTML);
    if (_.isEmpty(vodDetail.vod_content))
      vodDetail.vod_content = this.getDescription(/简介(.*?)<\/p>/gi, partHTML);
    if (_.isEmpty(vodDetail.vod_content))
      vodDetail.vod_content = this.getDescription(/◎简　　介(.*?)<br>/gi, partHTML);
      ...
      if (href_elements.length > 0) {
        ...
        this.total =
          this.page === this.count
            ? (this.page - 1) * this.limit + items.length
...
const { spider, __jsEvalReturn } = utilsClassFun(Xb6vSpider);
