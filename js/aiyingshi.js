...
class AiYingShiSpider extends Spider {
        ...
        for (const ele of $(elements[i]).find('a').slice(1)) {
          if ($($(elements[i]).find('a')[0]).text() === '全部类型') {
            extend_dic['value'].push({
              'n': $(ele).text(),
              'v': ele.attribs['href'].split('/').slice(-1)[0].split('.')[0],
            });
          } else {
  ...
  async getCateUrl(tid, pg, extend) {
    ...
    return (
      this.siteUrl + `/vodshow${time}${area}/id/${tid}${lanuage}${letter}${year}/page/${pg}.html`
    );
  }
...
const { spider, __jsEvalReturn } = utilsClassFun(AiYingShiSpider);
