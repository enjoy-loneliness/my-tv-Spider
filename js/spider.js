...
class Spider {
  ...
  async category(tid, pg, filter, extend) {
    ...
    await this.jadeLog.info(
      `正在解析分类页面,tid = ${tid},pg = ${pg},filter = ${filter},extend = ${JSON.stringify(
        extend
      )}`
    );
    if (tid === '最近更新') {
  ...
  async setDanmu(id) {
    ...
    await this.jadeLog.debug(
      `正在加载弹幕,视频详情为:${JSON.stringify(vodDetail)},集数:${JSON.stringify(
        this.episodeObj[id]
      )}`
    );
    //区分电影还是电视剧
  ...
  async getImg(url, headers) {
    ...
    try {
      ...
      if (vpn_proxy) {
        await this.jadeLog.error(
          `使用VPN代理,图片地址为:${url},headers:${JSON.stringify(
            headers
          )},代理失败,准备重连,输出内容为:${JSON.stringify(resp)}`
        );
      } else {
        await this.jadeLog.error(
          `使用普通代理,图片地址为:${url},headers:${JSON.stringify(
            headers
          )},代理失败,准备重连,输出内容为:${JSON.stringify(resp)}`
        );
      }
  ...
  async proxy(segments, headers) {
    await this.jadeLog.debug(
      `正在设置反向代理 segments = ${segments.join(',')},headers = ${JSON.stringify(headers)}`
    );
    let what = segments[0];
