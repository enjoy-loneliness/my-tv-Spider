...
class MoviePortSpider extends Spider {
  ...
  parseVodShortFromElement($, element) {
    ...
    vodShort.vod_remarks = $($(element).find('i')[0])
      .text()
      .replaceAll(' ', '')
      .replaceAll('\n', '');
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_director = Utils.getStrByRegex(
      /◎导　　演　(.*?)<br>/,
      $(vodDetailElement).html()
    );
    vodDetail.vod_content = Utils.getStrByRegex(
      /◎简　　介<\/p>(.*?)<br>/s,
      $(vodDetailElement).html()
    )
      .replaceAll('<p>', '')
    ...
    if (playFormatElement.length > 0) {
      ...
      vod_play_from_list.push(
        Utils.getStrByRegex(/【(.*?)】/, format_name.replaceAll('下载地址', '磁力链接'))
      );
      let vodItems = [];
...
const { spider, __jsEvalReturn } = utilsClassFun(MoviePortSpider);
