...
class JianPianSpider extends Spider {
  ...
  async setHomeVod() {
    let content = await this.fetch(
      this.siteUrl + '/api/tag/hand?code=unknown601193cf375db73d&channel=wandoujia',
      null,
      this.getHeader()
    );
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let categoryUrl =
      this.siteUrl +
      `/api/crumb/list?area=${area}&category_id=${cateId}&page=${pg}&type=0&limit=24&sort=${by}&year=${year}`;
...
const { spider, __jsEvalReturn } = utilsClassFun(JianPianSpider);
