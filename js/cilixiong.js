...
class CiliXiongSpider extends Spider {
  ...
  async parseVodShortListFromDoc($) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_pic = Utils.getStrByRegex(
        /background-image: url\('(.*?)'\)/,
        $(vodElement).find('[class="card-img"]')[0].attribs['style']
      );
  ...
  async parseVodShortListFromDocBySearch($) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_remarks =
        '评分:' + $($(vodElement).find('[class="rank bg-success p-1"]')).text();
      vod_list.push(vodShort);
    }
    return vod_list;
  }
  ...
  async parseVodDetailFromDoc($) {
    ...
    let vodDetailElement = $(
      '[class="row row-cols-1 row-cols-lg-3 align-items-stretch g-4 p-5 text-white"]'
    );
    ...
    let mangetIndex = 1;
    let mangetElements = $('[class="row col-md-12 text-white p-3 pt-1"]').find(
      '[class="container"]'
    );
...
const { spider, __jsEvalReturn } = utilsClassFun(CiliXiongSpider);
