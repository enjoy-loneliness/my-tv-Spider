...
class DyttSpider extends Spider {
  ...
  getSearchHeader() {
    ...
    headers['Cookie'] =
      'Hm_lvt_d8c8eecfb13fe991855f511a6e30c3d2=1708243467,1708325624,1708326536;Hm_lpvt_d8c8eecfb13fe991855f511a6e30c3d2;1708326536';
    return headers;
  }
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.type_name = $(
      $($(detailRootElement).find('[class="post-meta"]')).find('span')[0]
    ).text();
    ...
    for (let i = 0; i < downloadElements.length; i++) {
      ...
      let format_name = $($(playFormatElement).find('h2'))
        .text()
      ...
      for (const playUrlElement of $(downloadElements[i]).find('a')) {
        let episodeName = $(playUrlElement)
          .text()
          .replaceAll('磁力链下载', '')
          .replaceAll('.mp4', '');
  ...
  async parseVodShortListFromJson(obj) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_name = Utils.getStrByRegex(
        /《(.*?)》/,
        $(vodElement).find('img')[0].attribs.alt
      );
  ...
  async parseVodShortListFromDocByCategory($) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_name = Utils.getStrByRegex(
        /《(.*?)》/,
        $($($(vodElement).find('[class="text_info"]')).find('a')[0]).text()
      );
      vodShort.vod_pic = $(vodElement).find('img')[0].attribs['data-original'];
...
const { spider, __jsEvalReturn } = utilsClassFun(DyttSpider);
