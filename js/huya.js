...
class HuyaSpider extends Spider {
  ...
  async setFilterObj() {
    ...
    if (this.customArea.length > 0) {
      ...
      this.filterObj['custom'] = [
        { 'key': 'cateId', 'name': '分类', 'init': defKey.v, 'value': filterList },
      ];
    }
    if (this.isJustLive) {
      // key-value映射修改
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    if (this.isJustLive) {
      if (tid === 'home') {
        ...
        url =
          this.siteUrl +
          '/api/live/getRecommendByPlatformArea?platform=huya&size=20&area=' +
          extend.cateId +
          '&page=' +
          pg;
      }
      ...
      if (tid === 'home') {
        ...
        url =
          this.siteUrl +
          '/cache.php?m=LiveList&do=getLiveListByPage&gameId=' +
          extend['cateId'] +
          '&tagAll=0&page=' +
          pg;
      }
  ...
  async setDetail(id) {
    ...
    if (this.isJustLive) {
      ...
      const vodInfo = await this.fetch(
        this.siteUrl + `/api/live/getRoomInfo?platform=huya&roomId=${id}`,
        null,
        this.getHeader()
      );
      const playInfo = await this.fetch(
        this.siteUrl + `/api/live/getRealUrl?platform=huya&roomId=${id}`,
        null,
        this.getHeader()
      );
      ...
      const resp = await this.fetch(
        'https://mp.huya.com/cache.php?m=Live&do=profileRoom&roomid=' + id,
        null,
        this.getHeader()
      );
  ...
  async setSearch(wd, quick) {
    const resp = await this.fetch(
      'https://search.cdn.huya.com/?m=Search&do=getSearchContent&q=' +
        wd +
        '&uid=0&v=4&typ=-5&livestate=0&rows=40&start=0',
      null,
      this.getHeader()
    );
    const data = JSON.parse(resp);
...
const { spider, __jsEvalReturn } = utilsClassFun(HuyaSpider);
