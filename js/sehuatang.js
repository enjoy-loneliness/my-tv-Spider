...
class SHTSpider extends Spider {
  ...
  getHeader() {
    return {
      ...
      'Cookie':
        'cPNj_2132_saltkey=Q4BKEOEC; cf_clearance=6Gz2tvOXPkkJP2UhLnSsN4s0RrnDUy0jBN0kUvC5FNQ-1706109144-1-AebvwBnAURwWWQhj0QRBrRPku2n8xI73PIeuZVj2ckqY9zjQ7zFzDviX7Gkex1P1bUw9SXHGEYnkBB9nmWe6Nhk=; _safe=vqd37pjm4p5uodq339yzk6b7jdt6oich',
    };
  }
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_play_url =
      vodDetail.vod_name + '$' + Utils.getStrByRegex(/磁力链接: (.*)复制代码/, content);
    return vodDetail;
  }
  ...
  async getFilter($) {
    ...
    for (const themeElement of themeElements) {
      ...
      if (type_name !== '更多' && type_name !== '显示置顶') {
        extend_dic2['value'].push({
          'n': $(themeElement).text(),
          'v': themeElement.attribs['href'],
        });
      }
    }
...
const { spider, __jsEvalReturn } = utilsClassFun(SHTSpider);
