  ...
  async getApiUrl() {
    ...
    if (!_.isEmpty(html)) {
      ...
      return {
        'api':
          'https:' +
          Utils.getStrByRegex(/ return '(.*?)' \+ /, paper_js_content) +
          new Date().getTime(),
  ...
  async refreshToken() {
    ...
    if (_.isEmpty(this.token_dic['token'])) {
      ...
      await this.jadeLog.debug(
        `Token到期,上次获取Token时间为:${this_time},当前时间为:${parseInt(
          this.token_dic['date']
        )},刷新Token`
      );
      await this.get_token();
...
const { spider, __jsEvalReturn } = utilsClassFun(GitCafeSpider);
