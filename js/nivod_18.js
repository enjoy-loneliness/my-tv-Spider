...
class Nivod18Spider extends Spider {
    ...
    for (const cate_dic of obj) {
      for (const row of cate_dic.rows) {
        for (const cells of row.cells) {
          ...
          vodShort.vod_remarks = this.getVodRemarks(
            cells.show['hot'],
            cells.show['playResolutions']
          );
  ...
  getVodRemarks(hot, playResolutions) {
    ...
    if (this.catOpenStatus) {
      ...
      vod_remarks = `清晰度:${playResolutions[0]},热度:${Math.floor(
        parseInt(hot) / 1000
      ).toString()}k`;
    }
    return vod_remarks;
...
const { spider, __jsEvalReturn } = utilsClassFun(Nivod18Spider);
