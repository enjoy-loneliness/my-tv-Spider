...
class HaoXiSpider extends Spider {
  ...
  async parseVodShortListFromDoc($) {
    let vod_list = [];
    let vodElements = $('[class="flex bottom4"]').find(
      '[class="public-list-box public-pic-a [swiper]"]'
    );
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_year = $(
      $($(elements[0]).find('[class="slide-info-remarks"]')[0]).find('a')[0]
    ).html();
    vodDetail.vod_area = $(
      $($(elements[0]).find('[class="slide-info-remarks"]')[1]).find('a')[0]
    ).html();
    vodDetail.type_name = $(
      $($(elements[0]).find('[class="slide-info-remarks"]')[2]).find('a')[0]
    ).html();
  ...
  async setClasses() {
    ...
    for (const navElement of navElements) {
      ...
      let type_id = Utils.getStrByRegex(
        /\/vodtype\/(.*?)\//,
        $(navElement).find('a')[0].attribs.href
      );
...
const { spider, __jsEvalReturn } = utilsClassFun(HaoXiSpider);
