...
class TTKanSpider extends Spider {
  ...
  async parseVodShortListFromDocByCategory($) {
    let bookImgElements = $(
      '[class="pure-u-xl-1-5 pure-u-lg-1-4 pure-u-md-1-3 pure-u-sm-1-3 pure-u-13-24"]'
    );
    let bookMsgElements = $(
      '[class="pure-u-xl-4-5 pure-u-lg-3-4 pure-u-md-2-3 pure-u-sm-2-3 pure-u-11-24"]'
    );
    let books = [];
    for (let i = 0; i < bookImgElements.length; i++) {
      ...
      bookShort.book_remarks = $($(msgElement).find('li').slice(-1))
        .text()
        .replaceAll('状态：', '');
      books.push(bookShort);
    }
  ...
  async parseVodDetailFromDoc($, id) {
    ...
    let elements = $(infoElement)
      .find('[class="pure-u-xl-5-6 pure-u-lg-5-6 pure-u-md-2-3 pure-u-1-2"]')
      .find('li');
    ...
    bookDetail.book_year = $('[class="near_chapter"]')
      .find('time')[0]
      .attribs.datetime.replaceAll('T', ' ')
      .split('.')[0];
    ...
    let params = {
      'language': 'cn',
      'novel_id': id.replaceAll('/novel/chapters/', ''),
      '__amp_source_origin': encodeURIComponent(this.siteUrl),
    };
    let resJSon = JSON.parse(
      await this.fetch(this.apiUrl + '/nq/amp_novel_chapters', params, this.getHeader())
    );
    let allVodItems = [];
    for (const data of resJSon['items']) {
      const epName = data.chapter_name;
      const playUrl =
        epName + '+' + `${id.replaceAll('/novel/chapters/', '')}_${data.chapter_id}.html`;
  ...
  async setCategory(tid, pg, filter, extend) {
    if (tid === '/novel/rank') {
      ...
      let resJson = JSON.parse(
        await this.fetch(this.apiUrl + '/nq/amp_novel_list', params, this.getHeader())
      );
  ...
  async setPlay(flag, id, flags) {
    ...
    while (true) {
      ...
      content += Utils.formatContent(
        $('[class="content"]').html().trim().replaceAll('<p>', '    ').replaceAll('</p>', '\n')
      );
  ...
  async setPlay(flag, id, flags) {
    ...
    let bookContentList = $('[class="content"]')
      .text()
      .trim()
      .replaceAll('章节报错 分享给朋友：', '')
      .replaceAll('              ', '')
      .split('\n');
...
const { spider, __jsEvalReturn } = utilsClassFun(TTKanSpider);
