...
class ChangZhangSpider extends Spider {
  ...
  getSearchHeader() {
    return {
      'Cookie':
        'cf_clearance=otYZbHg1safCIxkCtZfy9DPKbf1Gs_zUskkVDc0MVKM-1707026063-1-ATOpKnTLv9+pv171YE/rzxN/nmvGN9Mucx7vpwp0kW2vZb/cbtz5e2md2/ym7EE+9dT7pPBV+kQOg9vJx2v8cks=;myannoun=1;PHPSESSID=73386nobqugs7r3pb2ljcsp5q4',
      'User-Agent':
        'Mozilla/5.0 (iPhone; CPU iPhone OS 16_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/100.0.4896.77 Mobile/15E148 Safari/604.1',
  ...
  async setClasses() {
    ...
    const series = $(
      'div#beautiful-taxonomy-filters-tax-movie_bt_series > a[cat-url*=movie_bt_series]'
    );
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    const link =
      this.siteUrl +
      '/movie_bt' +
      (tag.length > 0 ? `/movie_bt_tags/${tag}` : '') +
      '/movie_bt_series/' +
      tid +
      (pg > 1 ? `/page/${pg}` : '');
...
const { spider, __jsEvalReturn } = utilsClassFun(ChangZhangSpider);
