...
class NewVisionSpider extends Spider {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      for (const ele of $(elements[i]).find('a').slice(1)) {
        ...
        extend_dic['value'].push({
          'n': $(ele).text(),
          'v': decodeURIComponent(type_id_list[type_key_list[i]]),
        });
      }
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_pic = $($(vodDetailElement).find('[class="module-item-pic"]')).find(
      'img'
    )[0].attribs['data-src'];
    ...
    vodDetail.vod_content = $(
      $(itemElements[5]).find('[class="video-info-item video-info-content vod_content"]')
    )
      .text()
      .replaceAll('\n', '\t')
      .replaceAll('\t收起', '');
  ...
  async setPlay(flag, id, flags) {
    ...
    if (playUrl.indexOf('http') > -1) {
      ...
      //需要解析URL,支持弹幕
      let newUrl =
        'https://jiexi.xn--1lq90i13mxk5bolhm8k.xn--fiqs8s/player/ec.php?code=ak&if=1&url=' +
        playUrl;
...
const { spider, __jsEvalReturn } = utilsClassFun(NewVisionSpider);
