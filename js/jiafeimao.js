...
class JiaFeiMaoSpider extends Spider {
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_content = $(
      $('[class="idetail container"]').find('[class="infor_intro"]')
    ).text();
  ...
  async parseVodShortListFromDocBySearch($) {
    ...
    for (const vodElement of vodElements) {
      ...
      vodShort.vod_id = Utils.getStrByRegex(
        /id\/(.*?).html/,
        $($(vodElement).find('[class="s_tit"]')).find('a')[0].attribs.href
      );
  ...
  async setClasses() {
    ...
    for (const navElement of $(navContent).find('a')) {
      ...
      if (type_id !== '/' && type_name !== '专题' && type_name !== '站长模板') {
        this.classes.push(
          this.getTypeDic(type_name, Utils.getStrByRegex(/id\/(.*?).html/, type_id))
        );
      }
    }
  }
  ...
  async getFilter($) {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      let name = $($($(element).find('[class="v-tit"]')))
        .text()
        .replaceAll('：', '');
      if (name !== '频道') {
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let url =
      this.siteUrl + `/index.php/vod/show/${area}${sort}${id}${letter}${year}page/${pg}.html`;
  ...
  async setDetail(id) {
    let content = await this.fetch(
      this.siteUrl + '/api.php/provide/vod',
      {
        'ac': 'detail',
        'ids': id,
      },
...
const { spider, __jsEvalReturn } = utilsClassFun(JiaFeiMaoSpider);
