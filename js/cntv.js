...
class CNTVSpider extends Spider {
  constructor() {
    ...
    this.liveJsonUrl =
      'https://gh.con.sh/https://github.com/jadehh/LiveSpider/blob/main/json/live.json';
  }
    ...
    for (const data of dataList) {
      ...
      for (const extendData of data['items']) {
        if (data['classname'] === 'nianfen') {
          if (
            !this.arrayIsinclude('2024', data['items']) &&
            extendData['title'] !== '全部' &&
            !add_year_status
          ) {
  ...
  async getLiveUrl(channel_id, obj) {
    ...
    if (this.liveJson[channelName] !== undefined) {
      ...
      for (const data of obj['program']) {
        ...
        let episodeUrl =
          liveUrl + `?begintimeabs=${data['st'] * 1000}&endtimeabs=${data['et'] * 1000}`;
  ...
  async setHomeVod() {
    let resJson = JSON.parse(
      await this.fetch(
        this.apiUrl + '/api/page/iphone/HandheldApplicationSink/7.0.0/158',
        null,
        this.getHeader()
      )
    );
  ...
  async setDetail(id) {
    ...
    if (playType === 'play') {
      let resJson = JSON.parse(
        await this.fetch(
          `https://api.cntv.cn/video/videoinfoByGuid?serviceId=cbox&guid=${id}`,
          null,
          this.getHeader()
        )
      );
      ...
      let content = (
        await this.fetch(
          `https://api.cntv.cn/epg/epginfo3?serviceId=shiyi&c=${id}&cb=LiveTileShow.prototype.getEpg`,
          null,
          this.getHeader()
        )
      )
        .replaceAll('LiveTileShow.prototype.getEpg(', '')
        .replaceAll(');', '');
  ...
  async setPlay(flag, id, flags) {
    if (id.startsWith('http')) {
      ...
      this.playUrl =
        'https://hls.cntv.myhwcdn.cn/asp/hls/2000/0303000a/3/default/' + id + '/2000.m3u8';
    }
  }
}

const { spider, __jsEvalReturn } = utilsClassFun(CNTVSpider);
