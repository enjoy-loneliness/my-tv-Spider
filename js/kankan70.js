...
class Kankan70Spider extends Spider {
  ...
  async parseVodDetailFromDoc($) {
    ...
    vodDetail.vod_content = $(infoElement)
      .find('[class=des2]')
      .text()
      .replaceAll('\n', '')
      .replaceAll('剧情：', '');
  ...
  async setDetail(id) {
    ...
    if (!_.isEmpty(html)) {
      ...
      let mather =
        /<script type="text\/javascript" src="http:\/\/test.gqyy8.com:8077\/ne2(.*?)"><\/script>/g.exec(
          html
        );
...
const { spider, __jsEvalReturn } = utilsClassFun(Kankan70Spider);
