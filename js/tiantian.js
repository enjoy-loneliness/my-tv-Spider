...
class TianTianSpider extends Spider {
  ...
  async request(reqUrl, method, data) {
    ...
    if (res.code === 403) {
      ...
      this.cookie = _.isArray(res.headers['set-cookie'])
  ...
  async setPlay(flag, id, flags) {
    ...
    if (flag.indexOf('芒果') > -1 || flag.indexOf('腾讯') > -1 || flag.indexOf('爱奇艺') > -1) {
      if (!this.catOpenStatus) {
        this.danmuUrl = await this.danmuSpider.downloadDanmu(
          'https://dmku.thefilehosting.com/?ac=dm&url=' + id
        );
      }
    }
...
const { spider, __jsEvalReturn } = utilsClassFun(TianTianSpider);

export { spider, __jsEvalReturn, TianTianSpider };
