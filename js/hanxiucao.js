...
class HanXiuCaoSpider extends Spider {
      ...
      for (const layout of layoutObj['sortKeys']) {
        ...
        if (layout['label'] !== '综合排序') {
          ...
          extend_dic['value'].push({
            'n': layout['label'].toString(),
            'v': layout['value'].toString(),
          });
        }
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    let resJson = JSON.parse(
      await this.post(this.siteUrl + url, this.getParams(params), this.getHeader(), '')
    );
    if (type_id === 172) {
  ...
  async setSearch(wd, quick, pg) {
    ...
    let resJson = JSON.parse(
      await this.post(
        this.siteUrl + '/base/globalSearch',
        this.getParams(params),
        this.getHeader(),
        ''
      )
...
const { spider, __jsEvalReturn } = utilsClassFun(HanXiuCaoSpider);

export { spider, __jsEvalReturn, bt };
