...
class BilibiliSpider extends Spider {
  ...
  async parseVodDetailfromJson(obj, bvid) {
    ...
    let playUrlDatas = JSON.parse(
      await this.fetch(this.apiUrl + '/x/player/playurl', params, this.getHeader())
    );
    ...
    for (let j = 0; j < jSONArray.length; j++) {
      ...
      const playUrl =
        j + '$' + aid + '+' + cid + '+' + qualityList.join(':') + '+' + descriptionList.join(':');
    ...
    const relatedData = JSON.parse(
      await this.fetch(
        this.apiUrl + '/x/web-interface/archive/related',
        relatedParams,
        this.getHeader()
      )
    ).data;
  ...
  async setHomeVod() {
    ...
    let content = await this.fetch(
      this.apiUrl + '/x/web-interface/popular',
      params,
      this.getHeader()
    );
        ...
        if (dashjson.id === quality_id) {
          for (const key in this.vod_codec) {
            if (dashjson['codecid'] === key) {
              urls.push(
                Utils.base64Decode(quality_id) + ' ' + this.vod_codec[key],
                dashjson['baseUrl']
              );
            }
          }
  ...
  async checkLogin() {
    let result = JSON.parse(
      await this.fetch('https://api.bilibili.com/x/web-interface/nav', null, this.getHeader())
    );
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    url =
      this.apiUrl +
      `/x/web-interface/search/type?search_type=video&keyword=${encodeURIComponent(tid)}`;
  ...
  async proxy(segments, headers) {
    ...
    if (what === 'dash') {
      ...
      const str5 = ids[2];
      const urls =
        this.apiUrl + `/x/player/playurl?avid=${aid}&cid=${cid}&qn=${str5}&fnval=4048&fourk=1`;
      let videoList = '';
      let audioList = '';
...
const { spider, __jsEvalReturn } = utilsClassFun(BilibiliSpider);
