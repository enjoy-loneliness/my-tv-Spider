...
class KuaiKanSpider extends Spider {
  ...
  async request(reqUrl, postData, agentSp, get) {
    ...
    await this.jadeLog.debug(
      `URL:${reqUrl},headers:${JSON.stringify(headers)},data:${[JSON.stringify(postData)]}`
    );
  ...
  async setFilterObj() {
    for (const typeDic of this.classes) {
      ...
      if (typeId !== '最近更新') {
        let filterData = await this.request(this.siteUrl + '/api.php/Video/getFilterType', {
          type: typeId,
        });
  ...
  async setHomeVod() {
    let data = await this.request(this.siteUrl + '/api.php/Index/getHomePage', {
      'p': '1',
      'type': '1',
    });
      ...
      if (
        ...
        if (result.url) {
          if (result.url.indexOf('filename=1.mp4') > -1) {
            ...
            this.playUrl = await js2Proxy(
              true,
              this.siteType,
              this.siteKey,
              'lzm3u8/' + Utils.base64Encode(result.url),
              {}
            );
          }
        }
  ...
  async setSearch(wd, quick) {
    let data = await this.request(this.siteUrl + '/api.php/Search/getSearch', {
      key: wd,
      type_id: 0,
      p: 1,
    });
...
const { spider, __jsEvalReturn } = utilsClassFun(KuaiKanSpider);
