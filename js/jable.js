...
class JableTVSpider extends Spider {
  ...
  async getFilter($, index, type_id, type_name) {
    ...
    if (index < 4) {
      ...
      let type_seletc_list = [
        'div.img-box > a',
        '[class="horizontal-img-box ml-3 mb-3"] > a',
        '',
        'sort',
      ];
      ...
      for (const element of default$(type_seletc_list[index])) {
        ...
        let typeName = $($(element).find(type_id_select_list[index]))
          .text()
          .replaceAll('\t', '')
          .replaceAll('\n', '')
          .replaceAll(' ', '');
      ...
      for (let i = 0; i < navElements.length; i++) {
        let extend_dic = {
          'name': $($(navElements[i]).find('h2')).text(),
          'key': 'type',
          'value': [],
        };
  ...
  async parseVodShortListFromDoc($) {
    ...
    for (const element of vodElements) {
      ...
      if (vod_pic !== undefined) {
        ...
        if (
          !_.isEmpty(vodShort.vod_pic) &&
          vodShort.vod_remarks !== '[限時優惠]只需1元即可無限下載'
        ) {
  ...
  async parseVodDetailFromDoc($) {
    ...
    let episodeName = $($('[class="header-right d-none d-md-block"] > h6'))
      .text()
      .replaceAll('\n', '')
      .replaceAll('●', '');
    let vodItems = [];
  ...
  async setCategory(tid, pg, filter, extend) {
    ...
    if (tid.indexOf('latest-updates') > 1) {
      ...
      cateUrl =
        extend_type +
        `/${pg}/?mode=async&function=get_block&block_id=list_videos_common_videos_list&sort_by=${sort_by}&_=${new Date().getTime()}`;
    }
...
const { spider, __jsEvalReturn } = utilsClassFun(JableTVSpider);
